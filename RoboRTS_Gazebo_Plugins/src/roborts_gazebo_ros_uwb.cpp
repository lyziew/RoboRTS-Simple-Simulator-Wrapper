...
void GazeboRosUwb::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("gaussian_noise")){
    uwb_noise_pos_ = _sdf->GetElement("gaussian_noise")->GetValue()->Get(uwb_noise_pos_);
  }
  ...
  gt_publisher_ = node_handle_->advertise<geometry_msgs::PoseStamped>("ground_truth", 10);
...
{
  if (level == 1) {
    uwb_noise_pos_ = config.gaussian_noise_position;
  } else{
    config.gaussian_noise_position = uwb_noise_pos_;
  }
}
...
//  double dt = updateTimer.getTimeSinceLastUpdate().Double();
  ...
  gt_msg_.header.frame_id = frame_id_;
  gt_msg_.header.stamp = ros::Time::now();
  gt_msg_.pose.position.x = pose.pos.x;
  gt_msg_.pose.position.y = pose.pos.y;
  gt_msg_.pose.position.z = pose.pos.z;
  uwb_publisher_.publish(uwb_pose_msg_);
  gt_publisher_.publish(gt_msg_);
}

gazebo::math::Vector3 GazeboRosUwb::ErrorModel(math::Pose pose){
  gazebo::math::Vector3 pose_with_noise;
  pose_with_noise.x = pose.pos.x + SensorModelGaussianKernel<double>(0.0,0.03);
  pose_with_noise.y = pose.pos.y + SensorModelGaussianKernel<double>(0.0,0.03);
